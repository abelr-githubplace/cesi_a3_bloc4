...
namespace SaveManager
{
    ...
    public class SaveManager
    {
        public static bool Execute(Command command, Progress[] progresses, Config config)
        {
            ...
            {
                case Action.Save:
                    return Save(command.Saves, command.SaveType, progresses, config);
                case Action.Delete:
                    return Delete(command.Saves, command.DeleteFiles, config);
                case Action.Restore:
                    return Restore(command.Saves, progresses);
                default:
                    return false;
            }
        }
        ...
        private static bool Save(SaveInfo[] saves, SaveType? saveType, Progress[] progresses, Config config)
        {
            ...
            {
                var saver = new Saver.Saver(saves[i], effectiveType, progresses[i], config);
        ...
        private static bool Delete(SaveInfo[] saves, bool deleteFiles, Config config)
        {
            ...
            foreach (var save in saves)
            {
                ...
                if (deleteFiles)
                {
                    try
                    {
                        if (File.Exists(save.DestinationPath))
                            File.Delete(save.DestinationPath);
                        else if (Directory.Exists(save.DestinationPath))
                            Directory.Delete(save.DestinationPath, true);
                    }
        ...
        private static bool Restore(SaveInfo[] saves, Progress[] progresses)
        {
            ...
            for (int i = 0; i < saves.Length; i++)
            {
                var restorer = new Saver.Restorer(saves[i], progresses[i]);
                Task.Run(() => restorer.Start());
            }
            return true;
        }
